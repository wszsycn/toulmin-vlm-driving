#!/usr/bin/env python3
"""
train_dpo.py — Single-GPU DPO fine-tuning of the Toulmin SFT model on PSI.

Continues training the SFT LoRA adapter with DPO preference loss.

Reference model approximation: ref = base 4-bit weights with SFT adapter
disabled via model.disable_adapter().  This avoids storing a second copy of
adapter weights.  The approximation is common in QLoRA+DPO; the true SFT-based
ref is only better when the adapter is frozen (ours is trainable).

Usage:
    python train_dpo.py --config configs/toulmin_dpo.yaml
    python train_dpo.py --config configs/toulmin_dpo.yaml --smoke
"""
...
# ============================================================
# Video loading — identical to train_sft.py
# ============================================================
...
# ============================================================
# Label masking — identical to train_sft.py
# ============================================================
...
# ============================================================
# Vision-info helper — handles 2- and 3-return versions
# ============================================================
...
class DPOCollator:
    """
    Builds chosen and rejected encoded batches for each DPO record.

    The video appears in the user turn and is identical for chosen and rejected.
    Both sequences are processed through process_vision_info independently
    (same cached PIL frames → numerically identical pixel_values_videos).
    We store pixel_values_videos from the chosen pass and reuse it for the
    rejected forward pass — this saves ~6 MB per record at batch_size=1.
    """
    ...
    def _video_content(self, video_path: str) -> dict:
        ...
        if self.max_pixels is not None:
            vc["min_pixels"] = 4 * 28 * 28   # 3136 — below 112×112=12544
    ...
    def __call__(self, features: list[dict]) -> dict:
        ...
        for rec in features:
            ...
            # pixel_values_videos is identical for chosen/rejected (same video)
...
def get_response_logps(
    ...
    logits  = outputs.logits[:, :-1, :].float()  # cast to float32 for stable log_softmax
    targets = labels[:, 1:]
    mask    = (targets != -100)
    ...
    return (per_tok_logp * mask).sum(dim=-1)   # (batch,)
...
def dpo_loss(
    ...
    """Standard DPO loss. Returns (loss, chosen_rewards, rejected_rewards)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Single-GPU DPO fine-tuning of Toulmin SFT model on PSI"
    )
    ...
    # ── CSV log ────────────────────────────────────────────────────────────────
    log_path = Path(output_dir) / "dpo_step_losses.csv"
    with open(log_path, "w", newline="") as f:
        csv.writer(f).writerow([
            "opt_step", "loss",
            "chosen_logp", "rejected_logp",
            "chosen_reward", "rejected_reward",
            "reward_margin", "reward_accuracy", "lr",
        ])
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            # Smoke first-batch sanity check
            ...
            if global_step % grad_accum == 0:
                ...
                r_margin = (c_rew - r_rew).item()
                r_acc    = float(c_rew.item() > r_rew.item())
