...
OUTPUT_DIR = "/workspace/eval_outputs/cosmos_video_compare"
...
MAX_SAMPLES = 100   # 比如 100；None 表示全测
MAX_NEW_TOKENS = 96
...
def load_ft_model_and_processor(base_model_name: str, adapter_path: str, device_idx: int):
...
def run_inference_one(model, processor, sample: Dict[str, Any], max_new_tokens=96) -> str:
    # inference 只用 system + user，不用 GT assistant
...
def main():
    ...
    # ---------- FT model ----------
    print("\\n=== Evaluating FT model ===")
    ft_model, ft_processor = load_ft_model_and_processor(BASE_MODEL_NAME, FT_ADAPTER_PATH, FT_DEVICE)
    ...
    # ---------- Merge results ----------
    merged_results = []
    for sample, base_r, ft_r in zip(samples, base_outputs, ft_outputs):
        merged_results.append({
            ...
            "base_format_ok": extract_answer(base_r["pred_text"]) in ["yes", "no"],
            ...
            "ft_format_ok": extract_answer(ft_r["pred_text"]) in ["yes", "no"],
        })
    ...
    save_jsonl(merged_results, f"{OUTPUT_DIR}/compare_results.jsonl")
    save_csv(
        merged_results,
        f"{OUTPUT_DIR}/compare_results.csv",
        fieldnames=[
            "id", "video_path", "ground_truth_answer",
            "base_pred_answer", "base_format_ok",
            "ft_pred_answer", "ft_format_ok",
            "base_pred_text", "ft_pred_text"
        ]
    )
