...
SFT_ADAPTER_PATH = "/workspace/outputs/Cosmos-Reason2-8B-psi-video-sft"   # 改成你的SFT输出目录
...
MAX_SAMPLES = 4           # 先小一点跑通，再上200
SELECTED_FRAME_INDICES = [0]   # None = 全16张；可改成 [0, 5, 10, 15] 提速

NUM_GENERATIONS = 2        # judge成本高，第一版建议2
...
# 3. Dataset
# =========================================================
# def build_prompt_messages(image_paths: List[str]) -> List[Dict[str, Any]]:
#     user_content = []
...
# def build_grpo_dataset(jsonl_path: str, max_samples: Optional[int] = None, selected_frame_indices=None) -> Dataset:
#     rows = []
...
#             # ✅ 直接在这里加载图片，不用 set_transform
#             images = [Image.open(p).convert("RGB") for p in image_paths]


#             rows.append({
#                 "sample_id": sample_id,
#                 "prompt": build_prompt_messages(image_paths),
...
#     #     else:
#     #         # 单条样本
#     #         example["images"] = [Image.open(p).convert("RGB") for p in image_paths]
...
def build_grpo_dataset(jsonl_path: str, max_samples: Optional[int] = None, selected_frame_indices=None) -> Dataset:
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            # Pre-load images directly into the row
            images = [Image.open(p).convert("RGB") for p in image_paths]
...
class OpenAIJudge:
    ...
    def score(self, system_prompt: str, user_prompt: str, default_score: float = 0.0) -> Dict[str, Any]:
        ...
        try:
            # 使用 Responses API 的结构化输出思路：
            # 固定要求返回 JSON，字段只有 score 和 reason。
            # 如果你的 openai SDK 版本与这里略有差异，可只改这一小段调用。
...
# =========================================================
# 7. Load SFT model as GRPO init model
# =========================================================
...
# 关键：从SFT后的adapter继续训练，而不是从base重新开始
...
# CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 /workspace/train_psi_grpo_judge.py